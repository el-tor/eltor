...
#define TRUNNEL_EXT_TYPE_PAYMENT_HASH 3
...
/** Return a newly allocated trn_cell_extension_payment with all
...
/** Try to parse a trn_cell_extension_payment from the buffer in
 * 'input', using up to 'len_in' bytes from the input buffer. On
 * success, return the number of bytes consumed and set *output to the
 * newly allocated trn_cell_extension_payment_t. On failure, return -2
 ...
 * trn_cell_extension_payment in 'obj' is consistent. Return NULL if it
 * is, and a short message if it is not.
 */
...
/** As trn_cell_extension_payment_get_payment_hash, but take and return
 * a const pointer
 */
uint8_t trn_cell_extension_payment_getconst_payment_hash(const trn_cell_extension_payment_t *inp, size_t idx);
/** Change the element at position 'idx' of the fixed array field
 * payment_hash of the trn_cell_extension_payment_t in 'inp', so that
...
/** Return a pointer to the TRUNNEL_PAYMENT_HASH_LEN-element array
 * field payment_hash of 'inp'.
 */
