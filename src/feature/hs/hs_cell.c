...
/** Build and add the payment hash extension to the given extensions object.
                                  ...
                                  trn_extension_t *extensions)
{
  ...
  /* We are creating a cell extension field of type payment hash. */
  field = trn_extension_field_new();
  trn_extension_field_set_field_type(field, TRUNNEL_EXT_TYPE_PAYMENT_HASH);

  /* Build payment extension field. */
  payment_ext = trn_cell_extension_payment_new();

  /* Copy payment hash into payment extension cell. */
  ...
  /* We've just added an extension field to the cell extensions so increment
   * the total number. */
  trn_extension_set_num(extensions, trn_extension_get_num(extensions) + 1);
                               ...
                               const trn_extension_field_t *field)
{
  ...
  case TRUNNEL_EXT_TYPE_POW:
    ...
    break;
  case TRUNNEL_EXT_TYPE_PAYMENT_HASH:
    /* Payment hash for paid hidden services. */
    {
      trn_cell_extension_payment_t *payment_ext = NULL;
      if (trn_cell_extension_payment_parse(&payment_ext,
                trn_extension_field_getconstarray_field(field),
                trn_extension_field_getlen_field(field)) < 0) {
        log_fn(LOG_PROTOCOL_WARN, LD_REND, 
               "Invalid payment hash cell extension.");
        ret = -1;
        break;
      }
      /* Copy payment hash to rdv_data */
      ...
      log_info(LD_REND, "Received payment hash in INTRODUCE2 cell");
      trn_cell_extension_payment_free(payment_ext);
    }
    break;
  default:
